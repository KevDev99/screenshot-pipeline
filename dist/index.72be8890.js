...
function loadCategories(activeCategory = 1) {
    ...
    categories.map((category)=>{
        ...
        if (category.id === activeCategory) _categoryLi.classList.add("active");
...
function loadCategoryItems(activeCategoryId = 1) {
    ...
    const currentCategory = categories.find((category)=>category.id === activeCategoryId
    );
    // get the category items of the first
    const firstCategoryItems = categorieItems.find((categoryItem)=>categoryItem.categoryId === currentCategory.id
    );
    ...
    firstCategoryItems.items.map((categoryItem)=>{
        ...
        _categoryLi.classList.add("category-item");
        // if its the first one -> set to active
        if (gAlpaca[currentCategory.title] == categoryItem.id) _categoryLi.classList.add("active");
...
function setCategoryItem(categoryItemId) {
    ...
    const selectedCategory = categorieItems.find((categoryItem)=>categoryItem.categoryId == gCategoryId
    );
    // get category
    const category1 = categories.find((category)=>category.id == gCategoryId
    );
    // get selected category item
    const selectedCategoryItem = selectedCategory.items.find((categoryItem)=>categoryItem.id == categoryItemId
    );
    // get image part of the alpaca
    const _imagePart = document.getElementById(`alpaca-${category1.title}`);
...
function setActiveCategoryItem(categoryItemId) {
    ...
    for(var i = 0; i < _categoryItemsUlListItems.length; i++)if (_categoryItemsUlListItems[i].id == categoryItemId) {
        _categoryItemsUlListItems[i].classList.add("active");
        // add also to the glboal gAlpaca object to save the state
        const currentCategory = categories.find((category)=>category.id == gCategoryId
        );
        gAlpaca[currentCategory.title] = _categoryItemsUlListItems[i].id;
    } else _categoryItemsUlListItems[i].classList.remove("active");
}
function setActiveCategory(categoryId) {
    ...
    for(var i = 0; i < _categoryUlListItems.length; i++)if (_categoryUlListItems[i].id == categoryId) _categoryUlListItems[i].classList.add("active");
    else _categoryUlListItems[i].classList.remove("active");
}
...
function getRandomAlpaca() {
    ...
    categories.map((category)=>{
        ...
        let currCategoryItem = categorieItems.find((categoryItem)=>categoryItem.categoryId === category.id
        );
...
function downloadAlpaca() {
    html2canvas(_alpacaDiv, {
        height: _alpacaDiv.clientHeight - 10
